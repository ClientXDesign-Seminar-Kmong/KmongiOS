...
struct HomeAuthService {
    ...
    func homeHeader(completion: @escaping (NetworkResult<Any>) -> (Void)) {
        ...
        dataRequest.responseData { (response) in
            ...
            case .success:
                ...
                completion(judgeBannerData(status: statusCode, data: data))
    ...
    private func judgeBannerData(status: Int, data: Data) -> NetworkResult<Any> {
    ...
    func homeRecService(completion: @escaping (NetworkResult<Any>) -> (Void)) {
        let url = APIConstants.homeRecServiceURL
    ...
    private func judgeRecommendData(status: Int, data: Data) -> NetworkResult<Any> {
        let decoder = JSONDecoder()
        guard let decodedData = try? decoder.decode(GenericResponse<[HomeExpertData]>.self, from: data) else {
            return .pathErr
        }
        switch status {
        case 200:
